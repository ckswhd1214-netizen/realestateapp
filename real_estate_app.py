...
with tab1:
    ...
    with L:
        ...
        if invest_type == "실거주":
            ...
            loan_years    = st.selectbox("대출 만기", [10, 20, 30], index=1)
...
| **저평가율** | **{gp:+.2f}%** |
            ...
            b3.metric("취득세", f"{ci['취득세']:.3f}억")
            b4.metric("복비",   f"{ci['복비']:.3f}억")

            if ci["대출액"] > 0:
                st.info(f"대출 {ci['대출액']:.2f}억 | {loan_type} {loan_years}년 | 월 납입: {monthly:.3f}억 ({monthly*10000:.0f}만원) | 총 이자: {total_interest:.2f}억")
