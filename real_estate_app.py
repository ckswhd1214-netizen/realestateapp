...
def calc_V(rp, ap, lw, af, pr, jf, tf):
    base = rp * ap * lw * af * jf * tf
    premium = rp * ap * pr
...
def calc_trade_factor(all_trade_items, apt_name, area_py):
    """최근 3개월 vs 직전 3개월 거래량 비교"""
    now = datetime.now()
    recent = [(now - relativedelta(months=i)).strftime("%Y%m") for i in range(1, 4)]
    prev   = [(now - relativedelta(months=i)).strftime("%Y%m") for i in range(4, 7)]
    
    def count(months):
        return len([i for i in all_trade_items
                    if i["name"] == apt_name
                    and abs(i["area_m2"] / M2_TO_PYEONG - area_py) <= 2
                    and any(m in str(i.get("ym", "")) for m in months)])
    
    # all_trade_items에 ym 필드가 없으니 전체 건수로 단순 비교
    total = len([i for i in all_trade_items
                 if i["name"] == apt_name
                 and abs(i["area_m2"] / M2_TO_PYEONG - area_py) <= 2])
    
    if total >= 5:   return 1.03  # 거래 활발
    elif total >= 2: return 1.00  # 보통
    else:            return 0.97  # 거래 침체
...
with tab1:
    ...
    with R:
        if run:
            ...
            tf   = calc_trade_factor(all_trade_items, apt_name, area_py)
            V    = calc_V(rp, area_py, lw, af, pr, jf, tf)            
            A    = price_eok * 10000
            gp   = calc_gap(V, A)
...
| 거래량 보정 (Trade_Factor) | {tf:.2f}x |
                """)

            # 내재가치 반영 지표
            st.markdown("---")
            st.markdown("#### 📊 내재가치 반영 지표")
            f1, f2 = st.columns(2)
            f1.metric("전세가율 보정", f"{jf:.2f}x", 
                      delta="하방 강함" if jf > 1 else ("취약" if jf < 1 else "보통"))
            f2.metric("거래량 보정", f"{tf:.2f}x",
                      delta="거래 활발" if tf > 1 else ("침체" if tf < 1 else "보합"))
